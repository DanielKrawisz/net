...
namespace io {
    ...
    io::error catch_all (fun &&f, args &&...a) noexcept {
        ...
        } catch (const data::unimplemented &m) {
...
int main (int arg_count, char **arg_values) {
    std::signal (SIGINT, io::signal_handler);
    std::signal (SIGTERM, io::signal_handler);

    io::error err = io::catch_all (&io::main, data::slice<const char *const> {arg_values, arg_count});
    ...
    if (bool (err)) {
        ...
        if (static_cast<int> (err.Code) <= 5 && static_cast<int> (err.Code) > 0) {
            ...
            [] (io::error::code code, std::ostream &o) -> std::ostream & {
                switch (code) {
