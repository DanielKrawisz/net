...
#ifndef IO_WAIT_FOR_ENTER
#define IO_WAIT_FOR_ENTER
...
namespace io {
    ...
    void wait_for_enter (string message = "Press enter to continue...");
    ...
    bool get_user_yes_or_no (string message, maybe<bool> default_value = false);
    ...
    string get_user_password (string message, char mask = '*');

    /// @brief reads a decimal number from the user
    /// @param question question to ask the user.
    /// @return number returned as double
    double read_decimal (const string &question);
