...
#ifndef NET_HTTP_CLIENT
#define NET_HTTP_CLIENT

#include <stdlib.h>
#include <data/rate_limiter.hpp>
...
namespace net::HTTP {
    ...
    struct client {
        ...
        client (const HTTP::REST &rest, data::rate_limiter rate = {});
        client (ptr<HTTP::SSL> ssl, const HTTP::REST &rest, data::rate_limiter rate = {});
    ...
    private:
        ...
        // optional.
        data::rate_limiter Rate;

        // TODO add a timer to close the session if we wait too long.
        ptr<stream> Session;
    };
        
    inline client::client (const HTTP::REST &rest, data::rate_limiter rate) :
        client {std::make_shared<HTTP::SSL> (HTTP::SSL::tlsv12_client), rest, rate} {
            SSL->set_default_verify_paths ();
            SSL->set_verify_mode (asio::ssl::verify_peer);
        }

    inline client::client (ptr<HTTP::SSL> ssl, const HTTP::REST &rest, data::rate_limiter rate) :
