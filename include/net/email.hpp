...
namespace net::email {
    ...
    mailbox inline mailbox::addr_spec () const {
        ...
        if (x.data () == nullptr) throw data::exception {} << "invalid mailbox";
        return data::ASCII {x};
    }
    ...
    string_view inline header::name (string_view x) {
        ...
        if (z.size () < 2) throw data::exception {} << "invalid header";
    ...
    email::headers inline message::headers () const {
        return data::lift ([] (std::pair<string_view, string_view> x) -> email::header {
