...
namespace net::HTTP {
    ...
    request::make request::make::query (const ASCII &p) const {
        ...
        auto r = *this;
        r.Target.Query = std::make_shared<pctstr> (percent::encode (p));
        return r;
    }
    ...
    request::make request::make::fragment (const UTF8 &p) const {
        ...
        auto r = *this;
        r.Target.Fragment = std::make_shared<pctstr> (percent::encode (p));
        return r;
    }
    ...
    request::make request::make::host (const UTF8 &u) const {
        return add_headers ({entry<const header, ASCII> {header::host, percent::encode (u)}});
    }
