...
namespace net::percent {
    using data::encoding::percent::encode;
    using data::encoding::percent::decode;
}
...
namespace net {
    ...
    target::make target::make::query (const ASCII &q) const {
        if (bool (Query))
            throw data::exception {"URL error: query already set."};
    ...
    target::make target::make::fragment (const UTF8 &u) const {
        if (bool (Fragment))
            throw data::exception {"URL error: fragment already set."};

        make m = *this;
        m.Fragment = std::make_shared<pctstr> (percent::encode (u, ":#[]@=&"));
        return m;
    }
    ...
    target::make target::make::path (const net::path &p) const {
        if (Path != nullptr && *Path != pctstr {""})
            throw data::exception {"URL error: path already set."};
    ...
    URL::make::operator URL () const {
        if (!Protocol) throw data::exception {"invalid URI; no protocol given"};
        ...
        if (!u.valid ()) throw data::exception {} << "invalid URI: " << u;
    ...
    URL::make URL::make::protocol (const net::protocol &p) const {
        if (bool (Protocol))
            throw data::exception {"URL error: protocol already set."};

        if (!p.valid ())
            throw data::exception {"URL error: invalid protocol."};
    ...
    URL::make URL::make::port (const uint16 &u) const {
        if (bool (Port))
            throw data::exception {"URL error: port already set."};
    ...
    URL::make URL::make::address (const IP::address &ip) const {
        if (Host != nullptr)
            throw data::exception {"URL error: host already set."};

        auto version = ip.version ();
        if (version != 4 && version != 6)
            throw data::exception {} << "URL error: invalid IP address " << ip;
    ...
    URL::make URL::make::domain_name (const net::domain_name &name) const {
        ...
        if (!name.valid ())
            throw data::exception {} << "URL error: invalid domain name " << name;
    ...
    URL::make URL::make::registered_name (const UTF8 &z) const {
        ...
        make m = *this;
        m.Host = std::make_shared<pctstr> (percent::encode (z, ":/?#"));
        return m;
    }
    ...
    URL::make URL::make::user_name_pass (const UTF8 &username, const UTF8 &pass) const {
        ...
        if (UserInfo != nullptr)
            throw data::exception {"URL error: user info already set."};

        std::stringstream user_info;
        user_info << percent::encode (username, ":/@?#") << ":" << percent::encode (pass, ":/@?#");
    ...
    URL::make URL::make::user_info (const UTF8 &info) const {
        ...
        make m = *this;
        if (m.UserInfo != nullptr) throw data::exception {"URL error: user info already set."};

        m.UserInfo = std::make_shared<pctstr> (percent::encode (info, "/@?#"));
        return m;
    }

    path::path (list<UTF8> uf, char delim) {
        if (delim < 32 || delim >= 127 || strchr ("/?#[]%", delim) != nullptr)
            throw data::exception {} << "invalid path delimiter " << delim;

        std::string to_encode = std::string {"/?#[]%"} + delim;

        std::stringstream p;

        for (const UTF8 &u : uf) p << delim << percent::encode (u, ASCII {to_encode});
    ...
    list<UTF8> path::read (char delim) const {
        ...
        for (const string_view &z : zz) {
            auto meep = percent::decode (z);
            if (!meep) throw data::exception {} << "invalid percent-encoded string";
            x = x << *meep;
        }
    ...
    maybe<entry<UTF8, UTF8>> URL::user_name_pass () const {
        ...
        if (hh.data () == nullptr) return {};
        list<string_view> z = data::split (hh, ":");
        if (z.size () != 2) return {};
        return {entry<UTF8, UTF8> {*percent::decode (z[0]), *percent::decode (z[1])}};
    }

    maybe<dispatch<UTF8, UTF8>> read_query_map (string_view query) {
        list<string_view> z = data::split (query, "&");

        dispatch<UTF8, UTF8> params;

        for (const string_view &e : z) {
            list<string_view> p = data::split (e, "=");
            if (p.size () != 2) return {};
            params = params << entry<const UTF8, UTF8> {*percent::decode (p[0]), *percent::decode (p[1])};
        }
...
namespace net {
    ...
    template <> struct make_uri_action<pegtl::scheme> {
        template <typename Input>
        static void apply (const Input &in, URL::make &m) {
            if (m.Protocol == nullptr) m.Protocol = std::make_shared<pctstr> (in.string ());
        }
    };
    ...
    template <> struct make_uri_action<pegtl::user_info_at> {
        ...
        static void apply (const Input &in, URL::make &m) {
            if (m.UserInfo == nullptr)
                m.UserInfo = std::make_shared<pctstr> (in.begin (), in.begin () + in.size () - 1);
        }
    };
    ...
    template <> struct make_uri_action<pegtl::host> {
        template <typename Input>
        static void apply (const Input &in, URL::make &m) {
            if (m.Host == nullptr) m.Host = std::make_shared<pctstr> (in.string ());
        }
    };
    ...
    template <> struct make_uri_action<pegtl::port> {
        template <typename Input>
        static void apply (const Input &in, URL::make &m) {
            if (m.Port == nullptr) m.Port = std::make_shared<pctstr> (in.string ());
        }
    };

    template <> struct make_target_action<pegtl::path_after_authority> {
        template <typename Input>
        static void apply (const Input &in, target::make &m) {
            if (m.Path == nullptr) m.Path = std::make_shared<pctstr> (in.string ());
        }
    };
    ...
    template <> struct make_uri_action<pegtl::path_after_authority> {
        ...
        static void apply (const Input &in, URL::make &m) {
            if (!bool (m.Target)) m.Target = std::make_shared<target::make> ();
            if (m.Target->Path == nullptr) m.Target->Path = std::make_shared<pctstr> (in.string ());
        }
    };
    ...
    template <> struct make_target_action<pegtl::path_rootless> {
        ...
        static void apply (const Input &in, target::make &m) {
            if (m.Path == nullptr)
                m.Path = std::make_shared<pctstr> (in.string ());
        }
    };
    ...
    template <> struct make_uri_action<pegtl::path_rootless> {
        ...
        static void apply (const Input& in, URL::make &m) {
            if (!bool (m.Target))
                m.Target = std::make_shared<target::make> ();

            if (m.Target->Path == nullptr)
                m.Target->Path = std::make_shared<pctstr> (in.string ());
        }
    };
    ...
    template <> struct make_target_action<pegtl::query> {
        ...
        static void apply (const Input& in, target::make &m) {
            if (m.Query == nullptr)
                m.Query = std::make_shared<pctstr> (in.string ());
        }
    };
    ...
    template <> struct make_uri_action<pegtl::query> {
        ...
        static void apply (const Input& in, URL::make &m) {
            if (!bool (m.Target)) m.Target = std::make_shared<target::make> ();
            if (m.Target->Query == nullptr)
                m.Target->Query = std::make_shared<pctstr> (in.string ());
        }
    };
    ...
    template <> struct make_target_action<pegtl::fragment> {
        ...
        static void apply (const Input &in, target::make &m) {
            if (m.Fragment == nullptr)
                m.Fragment = std::make_shared<pctstr> (in.string ());
        }
    };
    ...
    template <> struct make_uri_action<pegtl::fragment> {
        ...
        static void apply (const Input& in, URL::make &m) {
            ...
            if (m.Target->Fragment == nullptr)
                m.Target->Fragment = std::make_shared<pctstr> (in.string ());
        }
    };
    ...
    URL::make URL::make::authority (const UTF8 &x) const {
        if (UserInfo || Host || Port)
            throw data::exception {"URL error: authority already set."};

        make m = *this;

        UTF8 encoded = percent::encode (x);
    ...
    template <> struct read_authority_action<pegtl::authority> {
        ...
        template <typename Input>
        static void apply (const Input &in, authority &p) {
            p = authority {std::string_view (in)};
        }
    };
    ...
    template <> struct read_ip_address_action<pegtl::ipv4> {
        ...
        template <typename Input>
        static void apply (const Input &in, IP::address &x) {
            x = IP::address {std::string_view (in)};
        }
    };
    ...
    maybe<ASCII> target::query () const {
        string_view sub;
    ...
    maybe<dispatch<UTF8, UTF8>> target::query_map () const {
        maybe<ASCII> q = query ();
    ...
    maybe<UTF8> target::fragment () const {
        string_view sub;
        ...
        if (sub.data () == nullptr) return {};
        return percent::decode (sub);
    }

    maybe<ASCII> authority::port () const {
        string_view sub;
    ...
    maybe<UTF8> authority::host () const {
        string_view sub;
    ...
    maybe<IP::address> authority::address () const {
        string_view sub;
        tao::pegtl::memory_input<> in (*this, "ip_address");
    ...
    maybe<uint16> authority::port_number () const {
        auto zp = this->port ();
        if (!zp) return {};
        maybe<uint16> u {0};
        *u = std::stoi (*zp);
    ...
    maybe<IP::TCP::endpoint> authority::endpoint () const {
        maybe<uint16> p = port_number ();
        maybe<IP::address> a = address ();
        if (!bool (p) || !bool (a)) return {};
        return IP::TCP::endpoint {*a, *p};
    }
    ...
    template <> struct read_uri_action<pegtl::uri> {
        template <typename Input>
        static void apply (const Input &in, URL &m) {
            m = URL {std::string_view (in)};
        }
    };
    ...
    template <> struct read_protocol_action<pegtl::scheme> {
        template <typename Input>
        static void apply (const Input &in, protocol &m) {
            m = protocol {std::string_view (in)};
        }
    };
    ...
    template <> struct read_protocol_action<pegtl::domain_name> {
        template <typename Input>
        static void apply (const Input &in, domain_name &m) {
            m = domain_name {std::string_view (in)};
        }
    };
    ...
    template <> struct read_target_action<pegtl::target> {
        template <typename Input>
        static void apply (const Input &in, target &m) {
            m = target {std::string_view (in)};
        }
    };
    ...
    template <> struct read_target_action<pegtl::endpoint> {
        template <typename Input>
        static void apply (const Input& in, IP::TCP::endpoint &m) {
            m = IP::TCP::endpoint {std::string_view (in)};
        }
    };

    std::istream &operator >> (std::istream &i, domain_name &dom) {
        data::parse::URL::domain_name m {};
        auto result = data::parse::read_token (i, m);

        if (i) dom = domain_name {result};
        return i;
    }

    std::istream &operator >> (std::istream &i, port &p) {
        data::parse::IP::port m {};
        auto result = data::parse::read_token (i, m);

        if (i) p = port {static_cast<uint16> (m.value)};
        return i;
    }

    std::istream &IP::operator >> (std::istream &i, IP::address &addr) {
        data::parse::IP::address m {};
        auto result = data::parse::read_token (i, m);

        if (i) addr = IP::address {result};
        return i;
    }

    std::istream &operator >> (std::istream &i, URL &u) {
        data::parse::URL::URI m {};
        auto result = data::parse::read_token (i, m);
    ...
    std::istream &operator >> (std::istream &i, authority &a) {
        data::parse::URL::authority m {};
        auto result = data::parse::read_token (i, m);

        if (i) a = authority {result};
        return i;
    }

    std::istream &operator >> (std::istream &i, target &targ) {
        data::parse::URL::target m {};
        auto result = data::parse::read_token (i, m);

        if (i) targ = target {result};
        return i;
    }

    std::istream &IP::TCP::operator >> (std::istream &i, IP::TCP::endpoint &ep) {
        data::parse::TCP::endpoint m {};
        auto result = data::parse::read_token (i, m);

        if (i) ep = IP::TCP::endpoint {result};
        return i;
    }
...
namespace data {
    ...
    string_view encoding::percent::URI::scheme (string_view x) {
        ...
        if (!tao::pegtl::parse<pegtl::uri_whole, net::read_scheme_action> (in, sub))
            return {};
    ...
    string_view encoding::percent::URI::authority (string_view x) {
        ...
        if (!tao::pegtl::parse<pegtl::uri_whole, net::read_authority_action> (in, sub)) return {};
        return sub;
    }
    ...
    string_view encoding::percent::URI::user_info (string_view x) {
        string_view sub;
        tao::pegtl::memory_input<> in (x, "user_info");
        if (!tao::pegtl::parse<pegtl::uri_whole, net::read_user_info_action> (in, sub)) return {};
        return sub.substr (0, sub.size () - 1);
    }
    ...
    string_view encoding::percent::URI::host (string_view x) {
        ...
        if (!tao::pegtl::parse<pegtl::uri_whole, net::read_host_action> (in, sub)) return {};
        return sub;
    }
    ...
    string_view encoding::percent::URI::port (string_view x) {
        ...
        if (!tao::pegtl::parse<pegtl::uri_whole, net::read_port_action> (in, sub)) return {};
        return sub;
    }
    ...
    string_view encoding::percent::URI::path (string_view x) {
        ...
        // in theory this will not happen.
        if (!tao::pegtl::parse<pegtl::uri_whole, net::read_path_action> (in, sub)) return {};
        return sub;
    }
    ...
    string_view encoding::percent::URI::query (string_view x) {
        ...
        if (!tao::pegtl::parse<pegtl::uri_whole, net::read_query_action> (in, sub)) return {};
        return sub;
    }
    ...
    string_view encoding::percent::URI::fragment (string_view x) {
        ...
        if (!tao::pegtl::parse<pegtl::uri_whole, net::read_fragment_action> (in, sub)) return {};
        return sub;
    }
    ...
    string_view encoding::percent::URI::address (string_view x) {
        ...
        if (!tao::pegtl::parse<pegtl::uri_whole, net::read_ip_address_action> (in, sub)) return {};
        return sub;
    }
    ...
    encoding::percent::URI encoding::percent::URI::normalize () const {
        ...
        for (char &x : *m.Protocol)
            x = std::tolower (x);

        if (m.Host) for (char &x : *m.Host)
            x = std::tolower (x);
...
namespace net::IP {
    ...
    address::operator bytes () const {
        ...
        list<data::uint16_big> ports_left {};

        // ports to the right of the "::"
        list<data::uint16_big> ports_right {};

        list<data::uint16_big> *ports = &ports_left;
        ...
        auto bi = b.begin ();
        for (data::uint16_big upl : ports_left) {
            *bi = upl[0];
            bi++;
            *bi = upl[1];
            bi++;
        ...
        for (data::uint16_big upr : ports_right) {
            *bi = upr[0];
            bi++;
            *bi = upr[1];
            bi++;
