...
namespace net::email {
    ...
    ASCII field::to_string (name n) {
        switch (n) {
            ...
            default: throw data::exception {} << "unknown field type.";
        }
    }

    list<std::pair<string_view, string_view>> message::headers (string_view x) {
        char empty_line[] = {13, 10, 13, 10};
        list<string_view> z = data::split (x, std::string {empty_line});

        if (z.size () != 2) throw data::exception {} << "invalid message";

        list<string_view> hz = data::split (z[0], string_view {CRLF, 2});
    ...
    string_view message::body (string_view x) {
        char empty_line[] = {13, 10, 13, 10};
        list<string_view> z = data::split (x, string_view {empty_line, 4});
    ...
    list<message::sent_info> message::sent () const {
        ...
        if (d.data () == nullptr)
            throw data::exception {"invalid message"};

        if (f.data () == nullptr)
            throw data::exception {"invalid message"};

        if (fff.size () != ddd.size ())
            throw data::exception {"invalid message"};

        if (i.data () == nullptr && iii.size () != 0 || i.data () != nullptr && iii.size () != fff.size ())
            throw data::exception {"invalid message"};

        if (s.data () == nullptr && sss.size () != 0 || s.data () != nullptr && sss.size () != fff.size ())
            throw data::exception {"invalid message"};

        if (t.data () == nullptr && ttt.size () != 0 || t.data () != nullptr && ttt.size () != fff.size ())
            throw data::exception {"invalid message"};

        if (c.data () == nullptr && ccc.size () != 0 || c.data () != nullptr && ccc.size () != fff.size ())
            throw data::exception {"invalid message"};

        if (b.data () == nullptr && bbb.size () != 0 || b.data () != nullptr && bbb.size () != fff.size ())
            throw data::exception {"invalid message"};
        ...
        while (fff.size () > 0) {
            ...
            if (ttt.size () > 0) {
                si.To = data::lift ([] (string_view x) -> address {
                    return address {x};
                }, data::split (ttt.first (), ","));
                ttt = ttt.rest ();
            }

            if (ccc.size () > 0) {
                si.Cc = data::lift ([] (string_view x) -> address {
                    return address {x};
                }, data::split (ccc.first (), ","));
                ccc = ccc.rest ();
            }

            if (bbb.size () > 0) {
                si.Bcc = data::lift ([] (string_view x) -> address {
                    return address {x};
                }, data::split (bbb.first (), ","));
                bbb = bbb.rest ();
            }
    ...
    maybe<list<address>> message::reply_to () const {
        ...
        return data::lift ([] (string_view x) -> address {
            return address {x};
        }, data::split (rt, ","));
    }
    ...
    maybe<list<msg_id>> message::in_reply_to () const {
        ...
        return data::lift ([] (string_view x) -> msg_id {
            return msg_id {x};
        }, data::split (irt, ","));
    }
    ...
    list<list<ASCII>> message::keywords () const {
        ...
        return data::lift ([] (string_view k) -> list<ASCII> {
            return data::lift ([] (string_view x) -> ASCII {
                return ASCII {x};
            }, data::split (k, ","));
        }, kkk);
    }
